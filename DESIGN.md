...
|---|---|---|
| `developerhub_application` | `APIMgmt.Applications` (OData, devportal) | Full CRUD. `app_key`/`app_secret` are generated by SAP on create and are write-once: they are stored as `Sensitive, Computed` attributes and never re-read on `Read` (Developer Hub does not echo secrets back), matching the "write-only secret" guidance in §20 of the brief. |
| `developerhub_product_subscription` | `APIMgmt.Applications(<id>)/ToSubscriptions` | Create/Read/Delete. No update endpoint is documented for a subscription in isolation; changing which product a subscription targets is a replace (`RequiresReplace` on `product_name`), which matches how subscriptions are modeled as an edge between an immutable application and an immutable product reference, not as a resource with independently mutable fields beyond what governance allows to change through the same create/delete flow SAP documents. |
...
- `developerhub_product_subscription`: composite ID
  `<application_id>/<product_name>`, matching the `ToAPIProduct` navigation
  key shown in the SAP sample payload
  (`APIMgmt.APIProducts('<ProdName>')`). Import:
  `terraform import developerhub_product_subscription.example <application_id>/<product_name>`.
...
document (`APIMgmt.Attributes`) follows plain OData v2-style
per-entity addressing: `GET`/`PUT`/`DELETE
Entity(key1=..,key2=..)`, with `x-csrf-token: fetch` required ahead of any
write. `developerhub_product_subscription` follows that same, already-proven
convention for its own CRUD, addressing the subscription as a keyed child of
`ToSubscriptions`. This is called out explicitly, both here and in the
resource's own doc comment, as a **documented inference** rather than a
verified fact — if SAP's actual behavior differs, the resource's `Read`
will surface that as a normal API error rather than crash, and the gap is
tracked in `FEATURE_MATRIX.md`.
