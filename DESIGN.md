...
| `api.sap.com` specs | – | **Not verified.** Reachable since 2026-09-25, but every specification download (and the catalog service `APIContent.APIs('<name>')/$value`) redirects to an interactive SAP Universal ID sign-in (`accounts.sap.com`), so it cannot be fetched non-interactively. To be checked against `Contract` from downloaded copies placed in `docs/api-specs/`: `DevPortal_Application_CF` ("Developer Hub - Application (CF)" — the primary one for `developerhub_application` and `developerhub_product_subscription`), `DevPortal_RegisteringUsers_CF`, `DevPortal_ExternalGovernance_CF`, `DevPortal_ExternalGovernanceSPI_CF`. |
